...
for example if the user wants to find all `Person` with `Name` `Kang Tze` or `Triston`, he must separate the names using two
...
Lastly, the user will enter the corresponding keywords to be searched under each attribute, in increasing order of their indexes.
