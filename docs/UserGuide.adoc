...
There are two types of Find Commands: Global Find and Specific Find, which can be used in conjunction with one another. +
Format: `find [PREFIX] KEYWORDS [NEXT_PREFIX] [MORE_KEYWORDS]`

[NOTE]
In order for a `Person` to show up on a `find`, the `Person` must match all attributes in the input. +
...
* no `Prefix` is included in the input.
...
* `find kang r/doctor` +
Displays all `Person` with `kang` in any of their attributes and whom is a `Doctor`.
* `find alex doctor` +
...
.Global Find Command with Specified Find Command
...
for example if the user wants to find all `Person` with `Name` `Kang Tze` or `Triston`, he must seperate the names using two
`Name` `Prefix`. e.g. `find n/Kang Tze n/Triston` displays `Ng Kang Tze` and `Triston Pang`.
* An illustration of a more complicated `find` command is `find friends n/Kang Tze n/Triston r/doctor`, which displays
all `Person` with `friends` in their attributes, has `name` containing either `Kang Tze` or `Triston`, and whom is a `Doctor`.
****

==== Intuitive Command Prompt (ICP) Support

ICP mode provides a more natural way of entering commands into OneLife. More information can be found under 3.18: Intuitive command prompting.
...
After entering into ICP Mode, the user will be prompted to select the attributes to be searched by entering the indexes
...
Lastly, the user will keywords to be searched under the attributes, in increasing numerical order.

The images below illustrate the utilisation of Find Command to execute the command `find doctor n/bernice` which means
`find` all `Person` with `doctor` in their attributes and whose name contains `bernice`, under ICP mode:
