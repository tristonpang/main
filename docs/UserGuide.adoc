...
* `find Goh` +
Displays `Gary Goh Yipeng` and `Kenneth Goh Jun Teck`.
* `find n/Tan a/Kang` +
Displays `Nelvin Tan` and all `Person` whose `Address` is `Choa Chu Kang`.
// end::find[]
...
.Entering an input
image::IntuitiveCommandPromptInputUserScreenshot.PNG[width="600"]
