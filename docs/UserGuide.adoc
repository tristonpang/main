...
* Index is the position of the person selected from the person card list.
* The index *must be a positive integer* `1, 2, 3, ...`
* Appointment List will be displayed on the top right of the screen as shown in figure below.
****

Examples:

* `list` (result shown in Figure 1) +
`select 2` (result shown in Figure 2) +
Selects the 2nd person in the database and display the appointments.
...
Schedules `John Doe` for an appointment with doctor `Jack` on the 23nd of November 2018 from
 1pm to 2pm.
* An example of a successful schedule is shown below.

.Successful schedule diagram
image::ScheduleSuccess.png[width="600"]
* An example of an unsuccessful schedule is shown below.
...
* `select 1` +
Displays the medical records of the first person on the displayed list as shown by the figure below.
...
Updates the medical records of the patient at index 1 with the given date, diagnosis and treatment without any comments as shown by the figure below.
...
Updates the medical records of the patient at index 1 with the given date, diagnosis, treatment and comments as shown by the figure below.
