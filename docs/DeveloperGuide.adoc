...
Step 4. Once the check is successful, we will have 2 objects, `personToEdit` and `Appointment` object.
We created another `Person` object called `editedPerson` that is equal to `personToEdit`.
We then add the `Appointment` to the `appointmentList` of `editedPerson`.
