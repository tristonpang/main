...
=== Creating a new command: `appointment`
...
==== Description
Edits the appointment for a person specified in the `INDEX`. +
Format: `appointment INDEX r/[REMARK]`

Examples:

* `appointment 1 r/Likes to drink coffee.` +
Edits the appointment for the first person to `Likes to drink coffee.`
* `appointment 1 r/` +
Removes the appointment for the first person.

==== Step-by-step Instructions

===== [Step 1] Logic: Teach the app to accept 'appointment' which does nothing
Let's start by teaching the application how to parse a `appointment` command. We will add the logic of `appointment` later.
...
. Add new test method to link:{repoURL}/src/test/java/seedu/address/logic/parser/AddressBookParserTest.java[`AddressBookParserTest`], which tests that typing "appointment" returns an instance of `RemarkCommand`.
