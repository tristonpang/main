...
For example: `schedule 1 d/23.11.2018 st/1300 et/1400 dn/Jack md/Heart pn/John Doe ic/S1234567A`
...
Step 3. `ScheduleCommand` will then be executed.
Various checks will be conducted to ensure that the appointment scheduled is valid.
...
** Whether the nric of the patient is valid.
** Whether the start and end times of the appointment are valid
** Whether there are any timing clashes between this appointment and the appointments currently stored in the appointment list under the person selected.

Step 4. A successful check will give 2 objects, `personToEdit` and `Appointment` object.
Program creates another `Person` object called `editedPerson` that is equal to `personToEdit`.
`Appointment` will be added to the `appointmentList` of `editedPerson`.
Lastly, we edit the `personToEdit` with `editedPerson` in our model.

image::ScheduleCommandStep4.png[width="800"]

Step 5. `appointmentList` will be updated in storage under `XmlAdaptedPerson`.
...
==== Design Considerations
An alternative considered is to use a comma-separated long string in the command prompt to schedule an appointment.
In our current implementation, we split up the various parts of an appointment into their respective prefixes.

For example:

`schedule 1 s/23.11.2018,1300,1400,Jack,Heart,John Doe,S1234567A`

** Pros: The user has to type less when scheduling a command.
The user is not required to type all the prefixes, he just has to type 1.
...
input is the `add` command word, the user's input is stored as a `commandWord` and the `IntuitivePromptManager`
triggers it's intuitive mode.

Step 5. `Model` then calls the `IntuitivePromptManager` to retrieve the next prompt to be shown to the user.
A String instruction representing the prompt to be shown is returned by `IntuitivePromptManager` and `Model` then
...
The following sequence diagram describes the above process:
...
Step 2-3. Remains the same as before with `patient` as the user's input.
...
// tag::display doctor's availability
=== Display doctor's current availability feature
==== Current implementation

The display of each doctor's current availability is facilitated by `PersonCard`, `Doctor`, `AppointmentManager`.
Given below is an example usage scenario and how the information is generated and propagated to be displayed in the
`PersonCard` at the final stage.

Step 1. The user launches the application for the first time. The `MainWindow` will be initialized and construct a
`PersonListPanel`.

Step 2. The `PersonListPanel` will then construct a list of `PersonCard`.

Step 3. Each `PersonCard` is associated with a person's particulars, be it a doctor or patient. In the event that
 this `PersonCard` belongs to a doctor, it will call the method `Doctor#currentAvailStatus` to get information on
 the doctor's current availability.

Step 4. Upon calling the method `Doctor#currentAvailStatus`, a helper method in `AppointmentManager`, called
`AppointmentManager#isAnyAppointmentOngoing` will be executed.

Step 5. `AppointmentManager` will then get the current locale time from Java's `ZoneDateTime` class and compare the
current time with all the scheduled appointments that the doctor has, and determines if the doctor is currently
available.

Step 6. The status information generated in `AppointmentManager` will then be propagated back up to the `PersonCard`
which then sets a badge to reflect the availability status of that doctor.
...
* **Alternative 1 (current choice):** A badge on each doctor's card display that shows his/her current availability.
** Pros: Convenient for the user to refer to since the badge is always on the display.
** Cons: Does not integrate any form of auto-updating nor is there any dedicated command to allow the user to refresh
the information, thus the information provided can be outdated.
* **Alternative 2:** A separate command that the user can call upon to look up the current availability of each doctor.
** Pros: Able to call this method repeatedly/when needed and will be provided with the latest information.
** Cons: Have a separate command dedicated to this feature which may not be necessary and not as convenient as compared
 to having a badge that is on the display at all times. Also this adds on to the list of commands
 that the user has to get familiar with.

===== Aspect: How to update the status of each doctor's availability

* **Alternative 1 (current choice):** Updates the status/badge only on launch of application and when there is a
change in doctor's appointment.
** Pros: Easy to implement.
** Cons: Information is outdated aas it does not truly reflect the doctor's current availability.
* **Alternative 2:** A function that will refresh the badge every few minutes.
** Pros: Provides accurate information on the doctor's availability.
** Cons: Affects the performance of the application due to this function continuously running in the background.
// end::display doctor's availability
...
Step 4. If the checks are successful, all the attributes of all the attributes of the specified person will first be copied, including the existing medical record library of the specified `Patient`.

Step 5. The new `MedicalRecord` will be added into the copied medical record library. The medical record library is an `ArrayList` of `MedicalRecord`. The new `MedicalRecord` is added to the back of the list and the pointer to the latest `MedicalRecord` is updated.
...
** Pros: Easy to access the most recent record by retrieving the last `MedicalRecord` in the ArrayList.
...
Step 1. The user wants to find all `Person` with `Name` Alex by executing `find Alex`.
...
* **Alternative 1 (current choice):** Allow users to perform global search using any keywords.
** Pros: Allows for a powerful search function by allowing the user to search through all `Person` attributes.
** Cons: Users will not be able to filter the unwanted results.
* **Alternative 2:** Allow the use of tags to specify certain fields if needed.
** Pros: Allows for a more precise search.
** Cons: Increased inconvenience of having to tag search fields.
// end::findcommand[]
