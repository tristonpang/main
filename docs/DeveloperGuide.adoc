...
`ScheduleCommand` then conducts various checks to ensure that the appointment scheduled is valid.
If it is invalid, an exception message will be thrown.
The `Appointment` will not be scheduled.
The various checks are displayed below.
...
Lastly, we replace `personToEdit` with `editedPerson` in our model.
